...
版本号使用 Git tag（例如 `1.0.8-SNAPSHOT`）。Redis 和 RabbitMQ 依赖均为可选——仅在使用对应功能时才需引入。
...
> **注意：** `invalidate()` 通过 Redis `INCR` 生成单调版本号并广播，版本感知的对端会跳过旧版本。`invalidateAll()` **不调用** `INCR`——它以版本 `0L` 广播，所有对端无条件生效。

## TTL 参考

| 方法 | TTL 含义 | 默认值 |
|---|---|---|
| `get(key, reader, ttlMs)` | Caffeine 硬 TTL——到期直接淘汰 | 第 3 参数必传 |
...
| `putThrough(key, value, writer, ttlMs)` | 写入条目的 Caffeine 硬 TTL | 第 4 参数可选，缺省使用 `local-cache-ttl-minutes` |
