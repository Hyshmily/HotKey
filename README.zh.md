...
- **Redis 集合类型** — 通过 `writeInvalidate` 支持 List/Set/ZSet 增量写入，无需 `writeThrough`
...
Optional<String> r = hotKeyCache.peek("user:123"); // 仅 Caffeine L1 + 热点检测
```
...
`writeThrough` 需要传入完整新值来更新 L1，但集合的增量操作（LPUSH、SADD、ZADD）只修改单个元素——调用方无法获知全量新值。使用 `writeInvalidate` 在突变后失效 L1，下次 `get()` 自动回源 Redis，保证一致性。
